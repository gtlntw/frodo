...
variantPassFile= #pass information for each variant; if this file is not available, set all to 1.; variant_pass.txt
genesFile=genes.txt  #read gene file : gene names & start/end positions; genes.txt
weightsFile= #read weights for variants; e.g. 1 for nonsynonymous and 0 otherwise; if this information is missing, set all weights to 1. weights.txt
...
## Run SKAT test (ro=0)
./FB-SKAT_v2.1 ${pedigreeFile} ${variantPassFile} ${genesFile} ${weightsFile} ${resultFile} ${mendelianErrorsFile} ${npermut} ${MAF} ${mend_th} 0 ${min_mark}
## Change ro to "1" to run BURDEN test
./FB-SKAT_v2.1 ${pedigreeFile} ${variantPassFile} ${genesFile} ${weightsFile} ${resultFile} ${mendelianErrorsFile} ${npermut} ${MAF} ${mend_th} 1 ${min_mark}
