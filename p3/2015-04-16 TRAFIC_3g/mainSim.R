...
sim_result <- replicate(n_rep, {
  ...
  result.trap <- family.test(data=family_generated_3g, f=risk.variant.id)
  result.trafic.ext <- family.test.trafic.ext(data=family_generated_3g, f=risk.variant.id)
  
  
  #only report p.value
  c(result.trap[7], result.trafic.ext)
  
})
## Write out your results to a csv file
write.csv(data.frame(seed=seed, r=r, result.trap.3g=sim_result[1,], result.trafic.ext.3g=sim_result[2,]),
