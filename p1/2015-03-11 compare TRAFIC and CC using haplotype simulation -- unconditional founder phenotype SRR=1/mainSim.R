...
if(null==FALSE) b[risk.variant.id] <- r #effect size is log10 of its allele frequency
...
sim.result <- replicate(n_rep, {
  ...
  SRR <- 1 #speedup
  family_generated <- gene_family(family_strct=family_strct.2g2c, n=1000, haplotype.risk=haplotype.risk*SRR^.25)
...
write.csv(data.frame(seed=seed, r=r, mu=mu, sigma2=sigma2, p_dis=p_dis, risk.haplo.f=risk.haplo.f,
          ...
          , paste("res_",round(mu,3),"_",round(sigma2,3),"_",p_dis,"_",seed,".csv",sep=""), row.names=FALSE)
## R.miSniam
## End(Not run)
## Not run: