...
EH_comp_Popcontrol <- function(mu=1, sigma2=0, f=0.01, out="A", p_dis=0.01) {
...
power_comp.sas <- function(mu=1.2, sigma2=3, f=0.01, n_pair=50, alpha=10^-6, out="A", Popcontrol=F, p_dis=0.01) {
  N <- 4*n_pair
  ifelse(Popcontrol==F, P_A <-EH_comp(mu=mu, sigma2=sigma2, f=f, out=out),
         P_A <- EH_comp_Popcontrol(mu=mu, sigma2=sigma2, f=f, out=out, p_dis=p_dis)) ##proportion under HA
  p1 <- P_A[1]
  p2 <- P_A[2]
  p0 <- 0
  w1 <- 0.5
...
gene_case_control <- function(n_case_control_pair=1000, haplotype.risk) {
