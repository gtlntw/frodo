...
# set.seed(0718) #to make sure the same set of risk variants
##generate risk haplotypes
# risk.variant.id <- sort(sample(which(snp$FREQ1 < 0.004), 10))
risk.variant.id <- c(5,6,9,12,24,27,32,34,43,44)
...
(mean(haplotype.risk[risk.haplo.id]/b0_sqrt)) #mean relative risk
...
sim.result <- replicate(n_rep, {
  ...
  #test statistics
  true.result <- prop.test(table(S_hap[which(!S_hap=="D")], apply(2-sibpair_haplotype[, 6+risk.variant.id], 1, sum)[which(!S_hap=="D")]>0), correct=F)
  ...
  n_sample=1000
  data_sibpair <- 2-sibpair_haplotype[, 6+risk.variant.id] #only causal snp 
  ...
  c(true.result$p.value, haplotype.result[3], genotype.result[3])
})

sim.result

write.csv(data.frame(seed=seed, r=r, true=sim.result[1,], haplotype=sim.result[2,], genotype=sim.result[3,]), paste("res_",r,"_",seed,".csv",sep=""), row.names=FALSE)
## R.miSniam
## End(Not run)
## Not run: