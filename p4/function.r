...
if(FALSE) {
  ...
  seed=1000
  n_family=1000
  family_strct = "3g.3a.5u" #"2g.2a.2u" "2g.2f.2c" "3g.3a.4u"
  trait_mean=100
  r=0
...
#     )
  ...
  test.stat <- data.frame(do.call(rbind, test.stat))
  carrier.family.idx <- test.stat$family.idx
  n_info_family <- length(carrier.family.idx)
  ...
  cutoff <- dbinom(n_gr_median, size = n_info_family, prob = 0.5) #p-value
  prob <- dbinom(0:n_info_family, n_info_family, 0.5)
  p.value <- sum(prob[which(prob<=cutoff)])

  list(n_gr_median=n_gr_median, p.value=p.value, n_info_family=n_info_family)
}
# family.rank.test()
