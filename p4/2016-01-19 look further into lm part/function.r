...
#     )
  ...
  if(lm.test==T) { #fisher's method to combine two p-values
    #lm test on all founders and child in the family with non-carrier founder
  	...
  	#pedgene with all founders only
  	lm.idx.only.founder <- which(data$data_family$father==0 & data$data_family$mother==0)
		data.cc.diploid <- hap2dip(data=list(data_family=data$data_family[lm.idx.only.founder, ]), risk.variant.id=risk.variant.id, save.file=F)
	  result.pedgene <- pedgene(ped=data.cc.diploid$ped, geno=data.cc.diploid$geno)
  	p.value.only.founder <- result.pedgene$pgdf$pval.burden  	
